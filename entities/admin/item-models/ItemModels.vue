...
const data = ref([
  {
    ...
    genres: 'Романтика',
    count: '14562',
    user: 'Константинопольский К.В',
  },
  {
    id: '4599',
    genres: 'Повседневность',
    count: '14562',
    user: 'Константинопольский К.В',
  },
  {
...
function getRowItems(row: Row<unknown>) {
  return [
    {
    ...
    <div class="p-4 flex flex-wrap items-center justify-between gap-4">
      <div class="flex flex-wrap items-center gap-4">
        <UInput
          ...
          variant="soft"
          :model-value="
            table?.tableApi?.getColumn('genres')?.getFilterValue() as string
          "
          class="max-w-sm"
          placeholder="Романтика"
          @update:model-value="
            table?.tableApi?.getColumn('genres')?.setFilterValue($event)
          "
        >
          <template
            v-if="table?.tableApi?.getColumn('genres')?.getFilterValue()"
            #trailing
          >
            <UButton
              color="neutral"
              ...
              @click="table?.tableApi?.getColumn('genres')?.setFilterValue('')"
            />
          </template>
        </UInput>
        ...
        <u-select
          ...
          multiple
          :model-value="
            table?.tableApi?.getColumn('count')?.getFilterValue() as string[]
          "
          :items="items"
          :ui="{
            placeholder: 'text-highlighted font-semibold ',
            trailingIcon: 'text-highlighted',
            base: 'light:bg-[#FFFFFF] ring-0 rounded-[10px] min-w-[165px] font-semibold',
          }"
          @update:model-value="
            table?.tableApi?.getColumn('count')?.setFilterValue($event)
          "
        />
        ...
        <u-select
          ...
          :items="items"
          :model-value="
            table?.tableApi?.getColumn('user')?.getFilterValue() as string[]
          "
          :ui="{
            placeholder: 'text-highlighted font-semibold',
            trailingIcon: 'text-highlighted',
          ...
          @update:model-value="
            table?.tableApi?.getColumn('user')?.setFilterValue($event)
          "
        />
      </div>
