export async function up(knex) {
  ...
  await knex.schema.createTable('team_teammate', (table) => {
    table.uuid('team_id').unsigned().references('team.id').onDelete('CASCADE');
    table.uuid('user_id').unsigned().references('user.id').onDelete('CASCADE');
  ...
  await knex.schema.createTable('team_liker', (table) => {
    table.uuid('team_id').unsigned().notNullable().references('team.id');
  ...
  await knex.schema.createTable('team_comment', (table) => {
    ...
    table
      ...
      .references('team.id')
      .onDelete('CASCADE');
    table.uuid('parent_id').unsigned().nullable().references('team_comment.id');
  ...
  await knex.schema.createTable('team_comment_liker', (table) => {
    table
      .uuid('team_comment_id')
      .unsigned()
      .notNullable()
      .references('team_comment.id');
    table.uuid('liker_id').unsigned().notNullable().references('user.id');
