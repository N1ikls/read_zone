export async function up(knex) {
  ...
  await knex.schema.createTable('complaints', (table) => {
    ...
    table
      ...
      .notNullable()
      .references('user.id')
      .onDelete('CASCADE'); // кто пожаловался
    table
      .uuid('target_user_id')
      .unsigned()
      .nullable()
      .references('user.id')
      .onDelete('CASCADE'); // на кого жалоба
    table
      .uuid('target_book_id')
      .unsigned()
      .nullable()
      .references('book.id')
      .onDelete('CASCADE'); // на какую книгу
    table
      .uuid('target_chapter_id')
      .unsigned()
      .nullable()
      .references('chapter.id')
      .onDelete('CASCADE'); // на какую главу
    ...
    table
      ...
      .notNullable();
    table
      .enu('status', ['pending', 'in_progress', 'resolved', 'rejected'])
      .notNullable()
      .default('pending');
  ...
  // Таблица уведомлений (создается только если не существует)
  // Примечание: расширенная версия создается в миграции 20250104000000_create_notifications_system.js
  ...
  await knex.schema.createTable('abandoned_requests', (table) => {
    ...
    table
      ...
      .notNullable()
      .references('user.id')
      .onDelete('CASCADE'); // кто запрашивает
    table
      .uuid('book_id')
      .unsigned()
      .notNullable()
      .references('book.id')
      .onDelete('CASCADE'); // какую книгу
    table.uuid('handled_by').unsigned().nullable().references('user.id'); // кто обработал
