...
export default class extends BaseStorage {
  ...
  get tableSubscriber() {
    return 'team_subscriber';
  }
  get tableTeammate() {
    return 'team_teammate';
  }
  ...
  beforeSave(team, user) {
    if (!team.id) {
      if (!('name' in team))
        throw new errors.DBValidation([
          { field: 'name', message: 'У команды должно быть название' },
        ]);
    }
    ...
    if ('name' in team) {
      if (typeof team.name !== 'string')
        throw new errors.DBValidation([
          { field: 'name', message: 'Название команды должно быть строкой' },
        ]);
      if (team.name.length === 0)
        throw new errors.DBValidation([
          { field: 'name', message: 'Название должно быть заполнено' },
        ]);
      data.name = team.name;
    }
      ...
      if (team.description === null) data.description = team.description;
        ...
        if (typeof team.description !== 'string')
          throw new errors.DBValidation([
            {
              field: 'description',
              message: 'Описание должно быть строкой или NULL',
            },
          ]);
